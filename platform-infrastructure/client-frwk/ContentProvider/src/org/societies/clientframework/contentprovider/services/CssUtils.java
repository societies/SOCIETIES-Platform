...
public class CssUtils {
	...
	public static ContentValues convertFromCssNode(CSSProfile profile){
		ContentValues data = new ContentValues();
	 ...
	 * @return
	 */
	public static List<CSSProfile> cursor2Profile(Cursor c, Map<String, CSSNode> nodes){
		List<CSSProfile> list = new ArrayList<CSSProfile>();
		try{
			if (c==null) return null;
			if (c.getCount()==0) return null;
			c.moveToFirst();
	...
	private  static String nodes2String(CSSNode[] nodes){
		String result="";
		for(CSSNode node :nodes){
			if (result.length()>0) result += ",";
			result+=node.getIdentity();
