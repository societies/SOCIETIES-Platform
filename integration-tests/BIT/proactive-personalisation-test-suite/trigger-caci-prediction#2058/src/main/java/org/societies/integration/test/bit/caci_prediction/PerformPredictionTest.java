...
public class PerformPredictionTest {
	...
	public void TestPerformPrediction() {
		...
		try {
			...
			this.cssOwnerId = TestCase2058.commMgr.getIdManager().fromJid(cssOwnerStr);
			...
			//LOG.info("CAUI PREDICTION SERVICE SET:"+ TestCase2058.cauiPrediction );
			
			//IndividualCtxEntity operator = TestCase2058.getCtxBroker().retrieveIndividualEntity(this.cssOwnerId).get();
			...
			List<IUserIntentAction> actionList = TestCase2058.cauiPrediction.getPrediction(this.cssOwnerId, actionRadio1).get();
	...
	protected CtxAttribute lookupRetrieveAttrHelp(String type){
		...
		try {
			List<CtxIdentifier> tupleAttrList = TestCase2058.ctxBroker.lookup(CtxModelType.ATTRIBUTE,type).get();
			if(tupleAttrList.size() >0 ){
				CtxIdentifier ctxId = tupleAttrList.get(0);
				ctxAttr =  (CtxAttribute) TestCase2058.ctxBroker.retrieve(ctxId).get();	
