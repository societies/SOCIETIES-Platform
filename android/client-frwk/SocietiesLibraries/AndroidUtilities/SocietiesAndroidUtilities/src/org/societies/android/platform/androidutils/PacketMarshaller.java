...
public class PacketMarshaller {
    ...
    private static final String LOG_TAG = PacketMarshaller.class.getCanonicalName();
...
//		Log.d(PacketMarshaller.class.getCanonicalName() + " ### ", xml);
...
//		Log.d(PacketMarshaller.class.getCanonicalName(), "Trying to unmarshall: " + packageStr + "." + beanName);
        Class<?> c = Class.forName(packageStr + "." + beanName);

        Object payload;

        try {
            payload = s.read(c, xml);
        } catch (Exception ex) {
            Log.e(LOG_TAG, "Error deserializing object", ex);
            throw ex;
        }
...
//		Log.d(PacketMarshaller.class.getCanonicalName() + " ### ", os.toString());
        return os.toString();
    }
